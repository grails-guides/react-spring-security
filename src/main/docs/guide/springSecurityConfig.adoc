...
In addition, the `s2-quickstart` has added some extra configuration at `server/grails-app/conf/application.groovy`. Remove `application.groovy. We want to keep all the configuration in a single file. Add the next snippet at the end of `aplication.yml`
