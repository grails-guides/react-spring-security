Let's continue by editing auth.js. This file is a "plain" JavaScript file (not a component), and will contain three functions (`logIn`, `logOut`, and `isLoggedIn`). We will be making use of HTML5's `localStorage` object to store the user's token after a successful login.
